<template>
  <div class="flex flex-col space-y-1 min-h-[150px]" :style="`width:${width}px;height: ${height}px`">
    <div ref="color" class="color flex-1 min-h-[120px]" :style="cptColor">
    ...
    <div class="scroll-y-hidden" :style="cptGridColor">
      <div
        v-for="(item, index) in state.preview"
        :key="index"
        :class="['shadow', { alpha: isAlpha(item), ping: state.delete && state.deleteList.includes(item) }]"
    ...
    <div class="h-5 between space-x-2">
      <span class="text-xs font-bold font-mono select-all">{{ props.color }}</span>
      <div class="flex space-x-2">
        <icon-svg
          name="delete"
          :class="['border rounded hover:bg-slate-100 active:bg-slate-200 p-[3px]', { 'border-blue-600 text-blue-600': state.delete }]"
          :style="`width: ${distance * 2}px;height: ${distance + 2}px`"
          @click="handleDeleteClick"
        />
        <icon-svg
          name="add"
          class="border rounded hover:bg-slate-100 active:bg-slate-200 p-[1px]"
          :style="`width: ${distance * 2}px;height: ${distance + 2}px`"
...
import { getValue, Hsva, parseColor, isAlpha } from "./color";
import { getPosition, setDragMove, getParseInt } from "./helper";
import { ref, reactive, computed, onMounted, PropType, toRef } from "vue";
...
const gap = 4;
const { cptGrid, distance } = useAutoColumn(toRef(props, "width"), { gap, width: 20 });
const cptGridColor = computed(() => ({ ...cptGrid.value, maxHeight: `${distance.value * 2 + gap}px` }));
...
const initColor = (pColor: string) => {
  ...
  if (props.color !== getValue(props.format, state.hsva)) updateColor();
};
...
const handleColorClick = (color: string) => {
  if (state.delete) {
    const index = state.deleteList.indexOf(color);
    if (index === -1) state.deleteList.push(color);
    else state.deleteList.splice(index, 1);
  } else {
    initColor(color);
    updateColor();
  }
...
const handleDeleteClick = () => {
  if (state.delete) {
    state.preview = difference(state.preview, state.deleteList);
    state.deleteList = [];
  }
...
.ping {
  @apply relative;
  &::before {
    @apply content-[''] block absolute bottom-0 right-0  border border-white w-2 h-2 rounded-full bg-black;
  }
}
