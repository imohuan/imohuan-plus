...
export const getValue = (format: "hsl" | "hsv" | "hex" | "rgb", hsva: Hsva) => {
  let color: string;
  const { h, s, v, a } = hsva;
  const isAlpha = Object.keys(hsva).includes("a") && (a as number) >= 0;
  switch (format) {
    case "hsl":
    ...
    default:
      ...
      if (format === "hex") {
        ...
        color = isAlpha ? `rgba(${r}, ${g}, ${b}, ${a})` : `rgb(${r}, ${g}, ${b})`;
      }
  }
  return color;
};

export const isAlpha = (color: string, threshold = 0.3) => {
  const hsva = parseColor(color);
  if (!hsva) return false;
  const alpha = hsva.a || 1;
