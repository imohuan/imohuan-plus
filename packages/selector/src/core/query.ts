...
export type ProcessingOption<T> = T & { [key: string]: any };
...
export interface QueryJsonOption<T> {
  value: string;
  rules: RuleItem[];
  parent: string;
  processing: (data: any, option?: ProcessingOption<T>) => any;
...
export function query<T = any>(
  ...
  let { cls, rules, parent, processing, char }: QueryOption<T> = defaultsDeep(userOption, {
    rules: [],
    replaces: [],
  ...
  clsObj.cls = parseReplace(clsObj.cls, defaultReplaces);
